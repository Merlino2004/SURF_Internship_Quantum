...
from helper_funcs_quantum_AFM import UtilityFunction, acq_max
...
class QBO(object):
    def __init__(self, f, pbounds, \
        ...
        self.X = np.array([]).reshape(-1, self.dim)
        self.Y = np.array([])
    ...
    def init(self, init_points):
        ...
        if isinstance(self.domain, np.ndarray) and self.domain.ndim == 2:

            random_indices = np.random.choice(self.domain.shape[0], size=init_points, replace=False)
            self.init_points = self.domain[random_indices].tolist()
        else:
    ...
    def maximize(self, n_iter=1000, init_points=1):
        ...
        self.x_list.extend(self.X.tolist())
        self.y_list.extend(self.Y.tolist())
        ...
        while self.total_used_queries < n_iter:
            ...
            self.x_list.append(x_max.tolist())
